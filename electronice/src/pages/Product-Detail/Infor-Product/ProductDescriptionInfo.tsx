...
function ProductDescriptionInfo(props: any) {
  console.log(props.product.variation)
        ...
        <div>
          ...
          {props.product.shortDescription?.split('. ').length > 1
            ? props.product.shortDescription?.split('. ').map((item: any, index: number) => {
                return (
                  <li style={{ listStyle: 'inside', fontWeight: 500, fontSize: 14, lineHeight: '28px' }} key={index}>
                    {item}
                  </li>
                )
              })
            : props.product.shortDescription?.split('. ').map((item: any, index: number) => {
                return (
                  <p style={{ fontWeight: 500, fontSize: 14, lineHeight: '28px' }} key={index}>
                    {item}
                  </p>
                )
              })}
              ...
              <div className="product-modal__content--size--content">
                {props.product.variation &&
                  props.product?.variation?.map((single: any) => {
                    console.log(single.color, selectedProductColor)
                ...
                dispatch(
                  addToCart({
                    ...
                    selectedProductColor: selectedProductColor
                      ? selectedProductColor
                      : props.product.selectedProductColor
                      ? props.product.selectedProductColor
                      : null,
                    selectedProductSize: selectedProductSize
                      ? selectedProductSize
                      : props.product.selectedProductSize
                      ? props.product.selectedProductSize
                      : null
                  })
                )
